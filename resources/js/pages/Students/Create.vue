...
const props = defineProps<{
    ...
    modelValue: boolean; // v-model:open bilan bog'lash uchun
}>();
...
const form = useForm({
    full_name: '',
    phone: '',
    birth_date: '' as string, // Sana string formatida (YYYY-MM-DD)
    balance: 0,
    group_ids: [] as number[],
});

// --- Popover holatlari ---
const openGroupSelect = ref(false); // Guruh tanlash popoveri
const openBirthDateCalendar = ref(false); // Tug'ilgan sana kalendari popoveri
...
const selectBirthDate = (date: Date) => {
    form.birth_date = format(date, 'yyyy-MM-dd'); // Backend uchun YYYY-MM-DD formatida saqlash
    openBirthDateCalendar.value = false; // Kalendarni yopish
};
...
const submit = () => {
    form.post(route('students.store'), {
        onSuccess: () => {
            ...
            openGroupSelect.value = false; // Guruh tanlash popoverini yopish
            openBirthDateCalendar.value = false; // Kalendarni yopish
            emit('update:modelValue', false); // Modalni yopish
            emit('success'); // Parent komponentga muvaffaqiyat haqida xabar berish
        },
        onError: (errors) => {
